...
class ClassService:
    @staticmethod
    def _class_ids(db: Session, current_user=None):
        if current_user is None or AccessScopeService.is_admin(current_user):
            return None
        if current_user.role == "teacher":
            return AccessScopeService.teacher_class_ids(db, current_user)
        return []
    ...
    @staticmethod
    def list_classes(db: Session, page: int = 1, size: int = 10, keyword: str = None, current_user=None):
        page = max(1, page)
        size = min(100, max(1, size))
        total, items = class_dao.get_classes(db, page, size, keyword, class_ids=ClassService._class_ids(db, current_user))
        return success({"items": items, "total": total, "page": page, "size": size}, "classes found")

    @staticmethod
    def get_class(db: Session, class_id: int, current_user=None):
        class_info = class_dao.get_class_by_id(db, class_id, class_ids=ClassService._class_ids(db, current_user))
        if not class_info:
